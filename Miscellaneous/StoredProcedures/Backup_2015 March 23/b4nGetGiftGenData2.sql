...
/****** Object:  Stored Procedure dbo.b4nGetGiftGenData2    Script Date: 23/06/2005 13:31:51 ******/

CREATE  PROCEDURE dbo.b4nGetGiftGenData2
@viewTypeID int,
@storeID int,
@Page int,
@Size int,
...
set @sql = @sql + '  from b4nproduct p with(nolock)  , b4nproductfamily f with(nolock) '
...
begin
 ...
 begin
  ...
  begin 
   ...
   set @sqlstep2 = @sqlstep2 + ' and cast(apf' + cast(@row as varchar(10)) + '.attributeValue as decimal) >= cast(' + cast(@startprice as varchar(100)) + ' as decimal)'
 
   if(@endprice != '0')
   begin
   set @sqlstep2 = @sqlstep2 + ' and cast(apf' + cast(@row as varchar(10)) + '.attributeValue as decimal ) <= cast(' + cast(@endprice as varchar(100)) + ' as decimal)'
   end
...
begin
   set @sqlstep2 = @sqlstep2 + ' and f.attributecollectionid = ' +  char(39) + cast(@attvalue  as varchar(2000)) + char(39) 
end
...
set @sqlstep1 = @sqlstep1 + '  where  p.deleted = 0  and p.productfamilyid = f.productfamilyid ' 
...
GRANT EXECUTE ON b4nGetGiftGenData2 TO b4nuser
GO
GRANT EXECUTE ON b4nGetGiftGenData2 TO helpdesk
GO
GRANT EXECUTE ON b4nGetGiftGenData2 TO ofsuser
GO
GRANT EXECUTE ON b4nGetGiftGenData2 TO reportuser
GO
GRANT EXECUTE ON b4nGetGiftGenData2 TO b4nexcel
GO
GRANT EXECUTE ON b4nGetGiftGenData2 TO b4nloader
GO
