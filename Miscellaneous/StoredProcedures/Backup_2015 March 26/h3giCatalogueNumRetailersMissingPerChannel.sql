...
--				for all retailers
-- =================================================
CREATE PROCEDURE [dbo].[h3giCatalogueNumRetailersMissingPerChannel] 
...
BEGIN
   ...
   --determine whether the handset is available for all retailers on contract
	SELECT	channel.channelCode, 
			ISNULL(missingRetailers.numberMissing,0) AS numberMissing
	FROM	h3giChannel channel WITH(nolock)
	LEFT OUTER JOIN
	(
		SELECT	COUNT(*) as numberMissing, 
				channelCode
...
GRANT EXECUTE ON h3giCatalogueNumRetailersMissingPerChannel TO b4nuser
GO
