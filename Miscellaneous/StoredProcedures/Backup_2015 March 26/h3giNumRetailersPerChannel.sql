...
-- =================================================
CREATE PROCEDURE [dbo].[h3giNumRetailersPerChannel] 
...
BEGIN
   ...
   --get the channels for which the product is available on contract
	SELECT channel.channelCode, ISNULL(totalRetailers.numRetailers,0) AS numberRetailers
	FROM h3giChannel channel WITH(nolock)
	LEFT OUTER JOIN 
	(
		SELECT COUNT(*) AS numRetailers, retailer.channelCode  
...
GRANT EXECUTE ON h3giNumRetailersPerChannel TO b4nuser
GO
