...
CREATE procedure [dbo].[h3giGetOrders_NC]
(  
	@Type as varchar(20),  
	@Future as bit = 0,  
...
begin  
	...
	set @sql = @sql + 'c.channelname as channel, h.channelCode, h.retailerCode, h.storeCode, o.status, cc.b4nclassdesc as statusDesc, r.retailername as nameofuser, od.depositAmount, h.OrderType as orderType, o.ccNumber, '  
	...
	Begin  
		...
		begin  
			...
			set @sql = @sql + ' and h.storecode <> ' + char(39) +  char(39) + ' '  
  			set @sql = @sql + ' and ((OH.orderStatus in (302, 402) AND OH.statusDate > '+ char(39) + cast(dateadd(dd,-7,getdate()) as varchar(25)) + char(39)  + ' ) '  
			set @sql = @sql + ' OR (OH.orderStatus not in (302, 402) AND OH.statusDate > '+ char(39) + cast(dateadd(dd,-5,getdate()) as varchar(25)) + char(39) + ' ) OR OH.StatusDate IS NULL)'  
		end  
